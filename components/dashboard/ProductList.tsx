        ...
        <div key={p.id} className="flex items-center gap-4 rounded-2xl glass p-4">
          ...
          <div className="min-w-0 flex-1">
            <p className="flex items-center gap-2 font-medium text-amethyst-50">
              {pick(p.name, locale)}
              {!p.active && <span className="rounded-full bg-white/10 px-2 py-0.5 text-[10px] text-amethyst-200/60">inactif</span>}
            </p>
            <p className="text-sm text-amethyst-200/60">
          ...
          <div className="flex gap-2">
            <Link href={`/tableau-de-bord/produits/${p.id}`} className="rounded-full border border-amethyst-300/25 px-4 py-1.5 text-xs text-amethyst-100 hover:bg-white/5">
              {dict.common.edit}
            </Link>
