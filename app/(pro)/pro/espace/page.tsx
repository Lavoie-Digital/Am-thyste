...
export default async function ProEspacePage() {
  ...
  const state =
    approved
      ? { label: dict.pro.title, title: dict.pro.approvedTitle, desc: dict.pro.approvedDesc, accent: "text-gold/70" }
      : viewer.proStatus === "rejected"
        ...
        : { label: dict.admin.status, title: dict.pro.pendingTitle, desc: dict.pro.pendingDesc, accent: "text-amethyst-300" };
    ...
    <div className="flex min-h-[80svh] items-center justify-center py-32">
      <Container className="max-w-xl">
        <div className="surface rounded-2xl p-10 text-center sm:p-14">
          ...
          <p className="mx-auto mt-5 max-w-md leading-relaxed text-amethyst-200/65">{state.desc}</p>
          ...
          {approved ? (
            ...
            <div className="mt-9">
              <Link href="/boutique" className="text-xs uppercase tracking-[0.18em] text-amethyst-200/60 transition-colors hover:text-ivory">
